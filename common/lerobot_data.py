...
def visualize_dataset(
    ...
    for batch in tqdm.tqdm(dataloader, total=len(dataloader)):
        ...
        for i in range(len(batch["index"])):
            ...
            if "state" in batch:
                for dim_idx, val in enumerate(batch["state"][i]):
                    rr.log(f"state/{dim_idx}", rr.Scalar(val.item()))

            if "jpos.cmd" in batch:
                for dim_idx, val in enumerate(batch["jpos.cmd"][i]):
                    rr.log(f"jpos.cmd/{dim_idx}", rr.Scalar(val.item()))
